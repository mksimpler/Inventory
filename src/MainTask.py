...
class MainTask(TaskBase):
    ...
    def location_prompt(self):
        ...
        if int(op_entry) != int(self._chk_digit):
            prompt_only(itext('main.check.digit', str(op_entry)), True)
            self.next_state = LOCATION_PROMPT
        
    def quantity_prompt(self):
        BACK_STOCK_CMD = 'back stock'
        self._bin_qty = prompt_digits(itext('main.quantity.prompt'),
                                      itext('main.quantity.prompt.help'),
                                      1, 5, True,
                                      additional_vocab = { BACK_STOCK_CMD: False })
        
        if self._bin_qty == BACK_STOCK_CMD:
            self._bin_qty = 0
            self.launch(obj_factory.get(BackStockTask, self._location, self.taskRunner),
                        self.current_state)
